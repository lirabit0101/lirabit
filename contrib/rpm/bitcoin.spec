...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9382
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9383
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19382
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19383
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 9382
	%{_sbindir}/semanage port -d -p tcp 9383
	%{_sbindir}/semanage port -d -p tcp 19382
	%{_sbindir}/semanage port -d -p tcp 19383
	for selinuxvariant in %{selinux_variants}; do
